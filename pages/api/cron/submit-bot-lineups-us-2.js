...
// Previous bots to check against (Bots 16-20)
...
async function runBotSubmissions() {
  console.log('🤖 Starting US bot lineup submissions (Bots 21-25)...');
  ...
  // Check what bots 16-20 submitted
  console.log('📊 Checking for previous bot lineups (Bots 16-20)...');
  ...
  for (let i = 0; i < BOT_USER_IDS.length; i++) {
    ...
    const result = await processSingleBot(botUserId, prompt.prompt, i + 21, submittedLineups, lockedHeadliner);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: `US bot submissions (21-25) complete: ${results.success.length} succeeded, ${results.failed.length} failed`,
      results
    });
  } catch (error) {
    console.error('Error running US bot submissions (21-25):', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
