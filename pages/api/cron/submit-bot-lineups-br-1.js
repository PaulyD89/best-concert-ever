...
async function runBotSubmissions() {
  console.log('🤖 Starting BR bot lineup submissions (Bots 31-35)...');
  ...
  // No previous bots for the first batch
  const PREVIOUS_BOT_IDS = [];
  
  console.log('📊 Checking for previous bot lineups...');
  ...
  for (let i = 0; i < BOT_USER_IDS.length; i++) {
    ...
    const result = await processSingleBot(botUserId, prompt.prompt, i + 31, submittedLineups, lockedHeadliner);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: `BR bot submissions (31-35) complete: ${results.success.length} succeeded, ${results.failed.length} failed`,
      results
    });
  } catch (error) {
    console.error('Error running BR bot submissions (31-35):', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
