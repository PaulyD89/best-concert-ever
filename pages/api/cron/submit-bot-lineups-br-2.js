...
// Previous bots to check against (Bots 31-35)
...
async function runBotSubmissions() {
  console.log('🤖 Starting BR bot lineup submissions (Bots 36-40)...');
  ...
  // Check what bots 31-35 submitted
  console.log('📊 Checking for previous bot lineups (Bots 31-35)...');
  ...
  for (let i = 0; i < BOT_USER_IDS.length; i++) {
    ...
    const result = await processSingleBot(botUserId, prompt.prompt, i + 36, submittedLineups, lockedHeadliner);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: `BR bot submissions (36-40) complete: ${results.success.length} succeeded, ${results.failed.length} failed`,
      results
    });
  } catch (error) {
    console.error('Error running BR bot submissions (36-40):', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
