...
async function runBotSubmissions() {
  console.log('🤖 Starting MX bot lineup submissions (Bots 1-5)...');
  ...
  for (let i = 0; i < BOT_USER_IDS.length; i++) {
    ...
    const result = await processSingleBot(botUserId, prompt.prompt, i + 1, submittedLineups, lockedHeadliner);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: `MX bot submissions (1-5) complete: ${results.success.length} succeeded, ${results.failed.length} failed`,
      results
    });
  } catch (error) {
    console.error('Error running MX bot submissions (1-5):', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
