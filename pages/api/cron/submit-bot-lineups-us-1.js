...
async function runBotSubmissions() {
  console.log('🤖 Starting US bot lineup submissions (Bots 16-20)...');
  ...
  for (let i = 0; i < BOT_USER_IDS.length; i++) {
    ...
    const result = await processSingleBot(botUserId, prompt.prompt, i + 16, submittedLineups, lockedHeadliner);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: `US bot submissions (16-20) complete: ${results.success.length} succeeded, ${results.failed.length} failed`,
      results
    });
  } catch (error) {
    console.error('Error running US bot submissions (16-20):', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
